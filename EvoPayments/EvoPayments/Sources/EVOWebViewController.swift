...
open class EVOWebViewController: UIViewController {
    ...
    public init(session: Session, statusCallback: @escaping EVOWebView.StatusCallback) {
    ...
    @objc private func dismissController() {
        if let unwrappedPresentationController = navigationController?.presentationController,
           #available(iOS 13.0, *) {
            unwrappedPresentationController
                .delegate?
                .presentationControllerWillDismiss?(unwrappedPresentationController)
        }
        navigationController?.dismiss(animated: true) { [weak self] in
            if let unwrappedPresentationController = self?.navigationController?.presentationController,
               #available(iOS 13.0, *) {
                unwrappedPresentationController
                    .delegate?
                    .presentationControllerDidDismiss?(unwrappedPresentationController)
            }
        }
    }
}
